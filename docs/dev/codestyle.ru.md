...
Ниже Вы можете увидеть некоторые наши особенности по стилистике кода, которых нет в Google's coding standard:

- Весь код должен соответствовать стандарту C++11.
...
- Includes сортируются и группируются по каталогам, между разными каталогами должны быть новые строки.
- Порядок каталогов в includes: `current_dir/current_file.hpp`, другие includes из того же каталога, includes из других каталогов, сортируются по имени.
...
- Мы используем все возможности C++11.
...
- Подчеркивания разрешены только в префиксах для переменных-членов и названиях пространств имен, например: `int m_key node Addr; namespace sc_utils`.
...
- Используйте порядок справа налево для переменных/параметров: `Srcaddr const & addr` (ссылка на константу `Srcaddr`).
...
- Пробел между двоичными операторами: `x = y * y + z * z`.
- Пробел после double dash `x = isValid ? 2 : 1;`.
- Мы используем ключевое слово `using` вместо `typedef`.
- Константы времени компиляции должны быть названы в camelCase, начиная со строчной буквы "k", например `kCompileTimeConstant`, и по возможности помечены как `constexpr`.
- Значения классов enum должны быть названы в camelCase, например `enum class Color { Red, Green, LightBlue };`.
- Макросы и enums в стиле C должны быть названы в UPPER_CASE, а к значениям enum должен быть добавлен префикс имени enum с заглавной буквы.
    Обратите внимание, что макросы усложняют отладку, а enums старого стиля содержат опасные неявные преобразования в целые числа и, как правило, загромождают содержащие их пространства имен. Избегайте их, когда это возможно - используйте `const` или `constexpr` вместо макросов и классы enum вместо enums.
...
Большая часть нашего стиля кодирования указана в файле конфигурации для [ClangFormat](http://clang.llvm.org/docs/ClangFormat.html).
...
## Форматирование Example/Guide/Reference
...
- Если у вас недостаточно времени, чтобы все исправить, оставьте комментарий `// TODO(имя_разработчика): нужно это исправить`.
...
- Проверка кода во время выполнения на наличие Debug builds:
...
- Проверка, компилируется ли код в режиме отладки:
...
### Logging
  * `SC_LOG_DEBUG(msg)` - вывод сообщения в Debug builds. Префикс: `[Debug]`
  * `SC_LOG_INFO(msg)`
  * `SC_LOG_WARNING(msg)`
  * `SC_LOG_ERROR(msg)`
